...
import { ArrowHeadFactory, S } from "./ArrowHeadFactory";
...
> {
  ...
  tmpGenerateEndMarker(id: string) {
    // console.log("tmpGenerateEndMarker ==>", this.props.link);
    return (
      <defs>
        ...
        >
          ...
          <S.Path
            selected={this.props.link.getOptions().selected}
            stroke={
              this.props.link.getOptions().selected
                ? this.props.link.getOptions().selectedColor
                : this.props.link.getOptions().color
            }
            d="M 0,1 3,3.5 0,6,z"
          />
  ...
  generateLink(
    ...
    const Top = React.cloneElement(Bottom, {
      ...
      onContextMenu: (event: MouseEvent) => {
        alert("You are at onContextMenu");

        // if (!this.props.link.isLocked()) {
        //   event.preventDefault();
        //   alert("You want delete link");
        //  // this.props.link.remove();
        // }
      }
    });

    //console.log("Bottom Top :",id);
    return (
  ...
  calculatePositions(
    ...
    console.log("calculatePositions", index);
  ...
  render() {
    ...
    let points = this.props.link.getPoints();
    console.log("path: 1 ==>", points.length);
    console.log("path: 2 ==>", points[1].getPosition());
    //    console.log("path: 3 ==>",points[2].getPosition());
    let paths = [];
    ...
    for (let j = 0; j < points.length - 1; j++) {
      // ss=points[j].getX(),points[j].getY()+","+points[j+1].getX(),points[j+1].getY();
      // if (j == 0) {
      //   ss = "M"+points[j].getPosition().x+","+points[j].getPosition().y+" L "+points[j].getPosition().x+10+","+points[j].getPosition().y;
      //   ss = ss+ " "+LinkWidget.generateLinePath(points[j], points[j + 1]).replace('M', 'L');;
      // } else {
      //   ss = ss + " " + LinkWidget.generateLinePath(points[j], points[j + 1]).replace('M', 'L');
      // }

      if (j == 0) {
        var x=points[j].getPosition().x;
        var y=points[j].getPosition().y;
        startValue=x+gap;
        ss = "M"+x+","+y+" L"+startValue+","+y;
        ss=ss+" L"+startValue+","+points[j+1].getPosition().y;
      }else{
        ss=ss+" L"+points[j+1].getPosition().x+","+points[j+1].getPosition().y;
      }
    ...
    // var str = "M395.8625081380208,109.875 L 395.8625081380208,267.53334554036445 L395.8625081380208,267.53334554036445 L 672.9499918619799,267.53334554036445";

    var ppp = roundCorners(ss, 14, 2);
    console.log("ppp :", ppp.path);

    for (let j = 0; j < points.length - 1; j++) {
      paths.push(
        this.generateLink(LinkWidget.generateLinePath(points[j], points[j + 1]),
          {
            "data-linkid": this.props.link.getID(),
            "data-point": j,
    ...
    let pathsNew = [];
    pathsNew.push(this.generateLink(ppp.path, {
      "data-linkid": this.props.link.getID(),
      "data-point": 0,
      // onMouseDown: (event: MouseEvent) => {
      //   if (event.button === 0) {
    ...
    console.log("generateLink :", pathsNew);
    return (
      <g
